        ...
        <section id={"work"}
            ...
            >
                {projects.map(({data:{image, description, title, link}}, index) => {
                    return (
                        <div key={index}
                             className="rounded-t-xl md:rounded-t-2xl relative bg-transparent hover:-translate-y-1 transition-transform duration-[500ms] max-h-[265px] md:max-h-[400px]">
                            <Link href={link}>
                                <Image src={image} alt={description} className="w-full min-h-[165px] lg:min-h-[400px] h-full object-cover rounded-2xl" priority/>
                                <div
                                    className="p-4 relative w-full -top-[6rem] dark:bg-[rgb(26,26,26)] bg-[#f7f7f7] rounded-b-xl md:rounded-b-2xl">
